...
function RecordDrawer({title,subtitle,onClose,children}:{title:string;subtitle:string;onClose:()=>void;children:React.ReactNode}){
  return <div className="drawer-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><aside className="record-drawer" role="dialog" aria-modal="true" aria-label={title}><button className="drawer-close" onClick={onClose} aria-label="Close">×</button><div className="drawer-heading"><Badge tone="blue">Demo record</Badge><h2>{title}</h2><p>{subtitle}</p></div>{children}</aside></div>
}
...
function ExternalPortal({ role, setRole, flash, notice }: { role: Exclude<Role,"fdi">; setRole: (role: Role) => void; flash: (s: string) => void; notice: string }) {
  const [tab, setTab] = useState(role === "vendor" ? "jobs" : "requests");
  ...
  return <main className={`external-shell ${isVendor ? "vendor-portal" : "customer-portal"}`}>
    ...
    <section className="external-content">
      ...
      <div className="portal-tabs">{(isVendor ? [["jobs","Available jobs"],["opportunity","My jobs"],["messages","Messages"],["schedule","Schedule"],["evidence","Completion"]] : [["new","New request"],["requests","My requests"],["request","Current request"],["proposal","Proposal"],["schedule","Schedule"],["updates","Updates"]]).map(([id,label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}>{label}</button>)}</div>
...
function VendorExperience({ tab, setTab, flash }: { tab: string; setTab: (tab: string) => void; flash: (s: string) => void }) {
  if (tab === "jobs") return <VendorJobs setTab={setTab} flash={flash} />;
  if (tab === "opportunity") return <VendorMyJobs flash={flash} setTab={setTab} />;
  ...
  if (tab === "schedule") return <div className="portal-two-col"><section className="portal-panel"><div className="portal-panel-title"><div><h2>Service schedule</h2><p>Coordinate arrival directly with FDI.</p></div></div><div className="big-date"><div><strong>28</strong><small>JUL</small></div><div><small>TUESDAY</small><h3>8:00–10:00 AM</h3><p>Parkview Commons · Loading dock</p></div><Badge tone="green">Confirmed</Badge></div><div className="assigned-location"><div className="mini-map"><span className="road r1"/><span className="road r2"/><span className="map-pin p-troy"><b>1</b></span></div><div><small>SERVICE ADDRESS</small><strong>2840 E. Maple Rd, Troy, MI</strong><p>Exact address is available because your company is assigned.</p><a href="https://www.google.com/maps/dir/?api=1&destination=2840+E+Maple+Rd+Troy+MI" target="_blank" rel="noreferrer">Open directions ↗</a></div></div><div className="access-note"><strong>Arrival instructions</strong><p>Check in through the work order on arrival. Site contact and access instructions will be released within 24 hours of service.</p></div><button className="secondary" onClick={() => flash("Reschedule request sent to FDI")}>Request a different time</button></section><VendorGuardrail /></div>;
...
function VendorJobs({ setTab, flash }: { setTab: (tab: string) => void; flash: (s: string) => void }) {
  type VendorJob={id:number;invitationId:number;invitationStatus:string;offeredAmount:number;customer_wo:string;title:string;description:string;city:string;address:string;facilityName:string;priority:string;service_window:string|null;assigned_vendor:string|null};
  const [jobs,setJobs]=useState<VendorJob[]>([]); const [loading,setLoading]=useState(true); const [selected,setSelected]=useState<VendorJob|null>(null);
  ...
  useEffect(()=>{load()},[]);
  async function respond(job:VendorJob,response:"accepted"|"declined"){await fetch("/api/demo",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"invitation_response",invitationId:job.invitationId,response})});flash(`${job.customer_wo} ${response} — FDI was notified`);setSelected(null);await load();if(response==="accepted")setTab("opportunity")}
  if(selected)return <VendorInvitationDetail job={selected} onBack={()=>setSelected(null)} onRespond={respond}/>;
  return <div className="jobs-layout"><section className="portal-panel"><div className="portal-panel-title"><div><h2>Jobs available to your company</h2><p>Private invitations matched to your trades, service area, compliance, and availability.</p></div><Badge tone="green">{loading?"Loading…":`${jobs.length} invitations`}</Badge></div><div className="job-invite-list">{jobs.map(job=><article key={job.invitationId} className={job.priority==="urgent"?"urgent":""}><div className="invite-main"><div><Badge tone={job.priority==="urgent"?"red":"blue"}>{job.priority==="urgent"?"Urgent":"Routine FM"}</Badge><small>{job.customer_wo} · Door systems</small></div><h3>{job.title}</h3><p>● {job.city} <span>·</span> ◷ {job.service_window||"Flexible"}</p></div><div className="invite-price"><small>AUTHORIZED AMOUNT</small><strong><Money value={job.offeredAmount}/></strong><span>Paid by FDI</span></div><div className="invite-actions"><button className="text-button" onClick={()=>respond(job,"declined")}>Decline</button><button className="secondary" onClick={()=>setSelected(job)}>View details</button><button className="primary" onClick={()=>respond(job,"accepted")}>Accept</button></div></article>)}</div>{!loading&&jobs.length===0&&<div className="empty-state"><strong>You’re all caught up</strong><p>New invitations matching your coverage will appear here. Accepted jobs are under My jobs.</p></div>}</section><FacilityMap vendor /></div>;
}

type PortalJob={id:number;invitationId:number;invitationStatus:string;offeredAmount:number;customer_wo:string;title:string;description:string;city:string;address:string;facilityName:string;priority:string;service_window:string|null;assigned_vendor:string|null};

function VendorInvitationDetail({job,onBack,onRespond}:{job:PortalJob;onBack:()=>void;onRespond:(job:PortalJob,response:"accepted"|"declined")=>void}){
  return <div className="portal-two-col"><section className="portal-panel"><button className="back-button" onClick={onBack}>← Back to available jobs</button><div className="portal-panel-title"><div><Badge tone={job.priority==="urgent"?"red":"blue"}>{job.priority==="urgent"?"Urgent invitation":"Routine FM invitation"}</Badge><h2 className="detail-title">{job.title}</h2><p>{job.customer_wo} · Private invitation from FDI</p></div><div className="invite-price"><small>AUTHORIZED AMOUNT</small><strong><Money value={job.offeredAmount}/></strong><span>Paid by FDI</span></div></div><div className="detail-grid"><div><small>TRADE</small><strong>Commercial door systems</strong></div><div><small>SERVICE AREA</small><strong>{job.city}</strong></div><div><small>REQUESTED WINDOW</small><strong>{job.service_window||"Flexible"}</strong></div><div><small>RESPONSE STATUS</small><strong>Awaiting your response</strong></div></div><div className="scope-box"><small>REQUESTED SCOPE</small><h3>{job.title}</h3><p>{job.description}</p></div><div className="privacy-note"><span>⌖</span><p><strong>Location privacy</strong> The approximate service area is shown before acceptance. The exact address and access instructions become available after FDI confirms the assignment.</p></div><div className="portal-actions"><button className="secondary" onClick={()=>onRespond(job,"declined")}>Decline invitation</button><button className="primary" onClick={()=>onRespond(job,"accepted")}>Accept job</button></div></section><FacilityMap vendor/></div>
}

function VendorMyJobs({flash,setTab}:{flash:(s:string)=>void;setTab:(s:string)=>void}){
  const [jobs,setJobs]=useState<PortalJob[]>([]); const [selected,setSelected]=useState<PortalJob|null>(null); const [loading,setLoading]=useState(true);
  useEffect(()=>{fetch("/api/demo?view=vendor").then(r=>r.json()).then(d=>setJobs((d.jobs||[]).filter((j:PortalJob)=>j.invitationStatus==="accepted"))).finally(()=>setLoading(false))},[]);
  if(selected)return <div className="portal-two-col"><section className="portal-panel"><button className="back-button" onClick={()=>setSelected(null)}>← Back to my jobs</button><div className="portal-panel-title"><div><Badge tone="green">Assigned</Badge><h2 className="detail-title">{selected.title}</h2><p>{selected.customer_wo} · {selected.facilityName}</p></div><div className="invite-price"><small>AUTHORIZED AMOUNT</small><strong><Money value={selected.offeredAmount}/></strong><span>Do not exceed</span></div></div><div className="detail-grid"><div><small>SERVICE ADDRESS</small><strong>{selected.address}, {selected.city}</strong></div><div><small>ARRIVAL WINDOW</small><strong>{selected.service_window||"Coordinate with FDI"}</strong></div><div><small>STATUS</small><strong>Assigned to your company</strong></div><div><small>FDI COORDINATOR</small><strong>Zak Keller</strong></div></div><div className="scope-box"><small>AUTHORIZED SCOPE</small><h3>{selected.title}</h3><p>{selected.description}</p></div><div className="portal-actions"><button className="secondary" onClick={()=>setTab("messages")}>Message FDI</button><a className="primary link-button" href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(`${selected.address}, ${selected.city}`)}`} target="_blank" rel="noreferrer">Open directions ↗</a></div></section><VendorGuardrail/></div>;
  return <div className="portal-two-col"><section className="portal-panel"><div className="portal-panel-title"><div><h2>My assigned jobs</h2><p>Accepted and active work for Great Lakes Door Co.</p></div><Badge tone="green">{loading?"Loading…":`${jobs.length} active`}</Badge></div><div className="request-list">{jobs.map(j=><button key={j.invitationId} onClick={()=>setSelected(j)}><span className="request-icon">⌂</span><div><small>{j.customer_wo} · {j.facilityName}</small><strong>{j.title}</strong><span>{j.service_window||"Coordinate with FDI"}</span></div><Badge tone="green">Assigned</Badge><div className="request-price"><small>AUTHORIZED</small><strong><Money value={j.offeredAmount}/></strong></div><i>→</i></button>)}</div>{!loading&&jobs.length===0&&<div className="empty-state"><strong>No active jobs</strong><p>Accept an available invitation to move it here.</p></div>}</section><VendorGuardrail/></div>
}
...
function CustomerExperience({ tab, setTab, flash }: { tab: string; setTab: (tab: string) => void; flash: (s: string) => void }) {
  if (tab === "new") return <NewCustomerRequest setTab={setTab} flash={flash} />;
  if (tab === "requests") return <CustomerRequests setTab={setTab} />;
...
function CustomerRequests({ setTab }: { setTab: (tab: string) => void }) {
  type CustomerJob={id:number;customer_wo:string;title:string;description:string;status:string;created_at:string;service_window:string|null;customer_price:number|null;nte:number;facilityName:string;city:string}; const [requests,setRequests]=useState<CustomerJob[]>([]); const [selected,setSelected]=useState<CustomerJob|null>(null); const [loading,setLoading]=useState(true);
  ...
  if(selected)return <div className="portal-two-col"><section className="portal-panel"><button className="back-button" onClick={()=>setSelected(null)}>← Back to my requests</button><div className="portal-panel-title"><div><Badge tone={tone(selected.status)}>{selected.status}</Badge><h2 className="detail-title">{selected.title}</h2><p>{selected.customer_wo} · Submitted {new Date(selected.created_at+"Z").toLocaleDateString()}</p></div></div><div className="detail-grid"><div><small>FACILITY</small><strong>{selected.facilityName} · {selected.city}</strong></div><div><small>ISSUED NTE</small><strong><Money value={selected.nte}/></strong></div><div><small>FDI PRICE</small><strong>{selected.customer_price?<Money value={selected.customer_price}/>:"Pending review"}</strong></div><div><small>NEXT STEP</small><strong>{selected.service_window||"FDI coordinator review"}</strong></div></div><div className="scope-box"><small>REQUESTED WORK</small><h3>{selected.title}</h3><p>{selected.description}</p></div><div className="approval-strip"><span>{selected.status==="Completed"?"✓":"↳"}</span><div><strong>{selected.status==="Completed"?"Work order completed":"FDI is managing this request"}</strong><p>{selected.customer_price?"Your FDI proposal and authorization are recorded on this request.":"Pricing, vendor selection, and scheduling are being coordinated by FDI."}</p></div></div><div className="portal-actions">{selected.customer_wo==="SC-847219"&&<button className="secondary" onClick={()=>setTab("proposal")}>View proposal</button>}<button className="primary" onClick={()=>setTab("updates")}>View updates</button></div></section><CustomerHelp/></div>;
  return <div className="portal-two-col"><section className="portal-panel"><div className="portal-panel-title"><div><h2>My service requests</h2><p>Track every request for facilities you are authorized to manage.</p></div><button className="primary" onClick={()=>setTab("new")}>＋ New request</button></div>{loading&&<div className="empty-state"><strong>Loading requests…</strong></div>}<div className="request-list">{requests.map(r=><button key={r.id} onClick={()=>setSelected(r)}><span className="request-icon">{r.status==="Completed"?"✓":"⌂"}</span><div><small>{r.customer_wo} · {new Date(r.created_at+"Z").toLocaleDateString()}</small><strong>{r.title}</strong><span>{r.service_window||"FDI coordinator reviewing"}</span></div><Badge tone={tone(r.status)}>{r.status}</Badge><div className="request-price"><small>FDI PRICE</small><strong>{r.customer_price?<Money value={r.customer_price}/>:"Pending"}</strong></div><i>→</i></button>)}</div></section><CustomerHelp /></div>;
}

function FacilityMap({ vendor=false }: { vendor?: boolean }) {
  return <aside className="portal-panel map-panel"><div className="portal-panel-title"><div><h2>{vendor?"Nearby opportunities":"Facility coverage"}</h2><p>{vendor?"Approximate locations before assignment.":"Live pilot facility locations."}</p></div></div><div className="map-canvas"><span className="road r1"/><span className="road r2"/><span className="road r3"/><span className="map-pin p-troy" title="Troy"><b>1</b></span><span className="map-pin p-royal" title="Royal Oak"><b>2</b></span><span className="map-pin p-novi" title="Novi"><b>3</b></span><div className="map-label l-troy">Troy</div><div className="map-label l-royal">Royal Oak</div><div className="map-label l-novi">Novi</div></div><div className="map-legend"><span><i/>Matched job</span><small>{vendor?"Exact address and access details unlock after assignment.":"3 facilities with active work"}</small></div></aside>;
}
...
function VendorsPage({ flash }: { flash: (s: string) => void }) {
  ...
  const [selectedVendor,setSelectedVendor]=useState<null|{initials:string;name:string;trade:string;territory:string;compliance:string;expiry:string;score:string;active:string}>(null);
  ...
  return <div className="content section-page"><div className="section-heading"><div><div className="eyebrow">APPROVED NETWORK · SOUTHEAST MICHIGAN</div><h1>Vendors</h1><p>Control coverage, compliance, and performance across FDI’s private network.</p></div><button className="primary" onClick={() => setInviteOpen(true)}>＋ Invite vendor</button></div>
    ...
    <section className="panel directory"><div className="directory-tools"><div className="search-box">⌕<input aria-label="Search vendors" placeholder="Search vendors, trades, or territory" /></div><div className="filter-tabs">{["All vendors","Compliant","Needs attention"].map(item => <button key={item} className={filter === item ? "active" : ""} onClick={() => setFilter(item)}>{item}</button>)}</div></div>
      ...
      {visible.map((v, i) => <button className="directory-row vendor-directory" key={v.name} onClick={() => setSelectedVendor(v)}><div className="vendor-cell"><span className={`vendor-logo v${i % 3}`}>{v.initials}</span><div><strong>{v.name}</strong><small>Approved FDI vendor</small></div></div><div><strong>{v.trade}</strong><small>{v.territory}</small></div><div><Badge tone={v.compliance === "Compliant" ? "green" : v.compliance === "Blocked" ? "red" : "amber"}>{v.compliance}</Badge><small>{v.expiry}</small></div><div><strong>{v.score} fit</strong><small>Quality & responsiveness</small></div><div><strong>{v.active}</strong><small>View details →</small></div></button>)}
    </section>
    {selectedVendor&&<RecordDrawer title={selectedVendor.name} subtitle="Approved FDI vendor profile" onClose={()=>setSelectedVendor(null)}><div className="detail-grid"><div><small>APPROVED TRADE</small><strong>{selectedVendor.trade}</strong></div><div><small>SERVICE TERRITORY</small><strong>{selectedVendor.territory}</strong></div><div><small>COMPLIANCE</small><strong>{selectedVendor.compliance}</strong></div><div><small>PERFORMANCE FIT</small><strong>{selectedVendor.score}</strong></div></div><div className="scope-box"><small>COMPLIANCE RECORD</small><h3>{selectedVendor.expiry}</h3><p>Insurance, licensing, service territory, and FDI performance notes are reviewed before every new assignment.</p></div><div className="portal-actions"><button className="secondary" onClick={()=>flash("Compliance review opened")}>Review compliance</button><button className="primary" disabled={selectedVendor.compliance==="Blocked"} onClick={()=>flash(selectedVendor.compliance==="Blocked"?"Assignment blocked by compliance":"Vendor selected for invitation")}>{selectedVendor.compliance==="Blocked"?"Assignments blocked":"Invite to work order"}</button></div></RecordDrawer>}
    {inviteOpen&&<RecordDrawer title="Invite a vendor" subtitle="Add a company to FDI’s private approved network" onClose={()=>setInviteOpen(false)}><div className="request-form"><label>Business name<input placeholder="Vendor company"/></label><label>Primary trade<select><option>Door systems</option><option>Unit cleanouts</option></select></label><label>Service territory<input placeholder="Example: 40 miles from Troy"/></label><label>Contact email<input placeholder="vendor@example.com"/></label></div><div className="portal-actions"><button className="secondary" onClick={()=>setInviteOpen(false)}>Cancel</button><button className="primary" onClick={()=>{setInviteOpen(false);flash("Vendor invitation saved as a demo record")}}>Send invitation</button></div></RecordDrawer>}
  </div>;
}

function CustomersPage({ flash }: { flash: (s: string) => void }) {
  const [drawer,setDrawer]=useState<null|{title:string;subtitle:string}>(null);
  return <div className="content section-page"><div className="section-heading"><div><div className="eyebrow">PILOT CUSTOMERS</div><h1>Customers & facilities</h1><p>Customer-specific work-order sources, approval rules, facilities, and service history.</p></div><button className="primary" onClick={() => setDrawer({title:"Add pilot customer",subtitle:"Configure customer intake and approval rules"})}>＋ Add customer</button></div>
    ...
    ].map(c => <button className="customer-card" key={c.name} onClick={() => setDrawer({title:c.name,subtitle:`${c.facilities} · ${c.source} intake`})}><div className="customer-top"><span className={`customer-logo ${c.color}`}>{c.code}</span><Badge tone="green">Active pilot</Badge></div><h3>{c.name}</h3><p>{c.place}</p><div className="customer-metrics"><div><small>Portfolio</small><strong>{c.facilities}</strong></div><div><small>Current work</small><strong>{c.open}</strong></div><div><small>30-day billing</small><strong>{c.spend}</strong></div></div><div className="source-chip"><span>↳</span><div><small>Primary intake</small><strong>{c.source}</strong></div><i>→</i></div></button>)}</div>
    <div className="fdi-map-row"><FacilityMap/><section className="panel facility-list"><div className="panel-title"><div><h2>Locations with active work</h2><p>Select a facility to review requests and coverage.</p></div><Badge tone="blue">Map view</Badge></div>{[["1","Parkview Commons","Troy · 4 open jobs"],["2","Woodward Flats","Royal Oak · 2 open jobs"],["3","Westgate Plaza","Novi · 1 open job"]].map(x=><button key={x[0]} onClick={()=>setDrawer({title:x[1],subtitle:x[2]})}><span>{x[0]}</span><div><strong>{x[1]}</strong><small>{x[2]}</small></div><i>→</i></button>)}</section></div>
    <section className="panel activity-panel"><div className="panel-title"><div><h2>Facility activity</h2><p>Recent work across the pilot portfolio.</p></div><button className="text-button" onClick={()=>setDrawer({title:"All pilot work orders",subtitle:"7 active work orders across 8 facilities"})}>View all work orders</button></div>{[
    ...
    ].map((row,i) => <button className="activity-row" key={row[0]} onClick={() => setDrawer({title:row[1],subtitle:`${row[0]} · ${row[2]}`})}><span className="facility-mark">⌂</span><div><strong>{row[0]}</strong><small>{row[1]}</small></div><Badge tone={i===0?"green":i===1?"blue":"amber"}>{row[2]}</Badge><div><strong>{row[3]}</strong><small>{row[4]}</small></div><span>→</span></button>)}</section>
    {drawer&&<RecordDrawer title={drawer.title} subtitle={drawer.subtitle} onClose={()=>setDrawer(null)}><div className="detail-grid"><div><small>INTAKE SOURCE</small><strong>ServiceChannel / customer portal</strong></div><div><small>RESPONSIBLE FMC</small><strong>Zak Keller</strong></div><div><small>DIVISION</small><strong>Routine facilities maintenance</strong></div><div><small>COVERAGE</small><strong>Southeast Michigan</strong></div></div><div className="scope-box"><small>WORKSPACE</small><h3>Facilities, requests, approvals, and service history</h3><p>This record brings the selected customer or facility context into one place. The production version will use the connected customer and facility records.</p></div></RecordDrawer>}
  </div>;
}

function AccountingPage({ flash }: { flash: (s: string) => void }) {
  const [accountingFilter, setAccountingFilter] = useState("Needs action");
  const [selectedRow,setSelectedRow]=useState<null|{wo:string;job:string;customer:string;vendor:string;vendorInvoice:string;customerInvoice:string;status:string;tone:"green"|"amber"|"blue"}>(null);
  const rows = [
  ...
  return <div className="content section-page"><div className="section-heading"><div><div className="eyebrow">OPERATIONAL ACCOUNTING</div><h1>Accounting</h1><p>Prepare clean handoffs to QuickBooks Online and track status without moving money.</p></div><button className="primary" onClick={() => flash("QuickBooks handoff report prepared")}>Export QBO handoff</button></div>
    ...
    <section className="panel directory"><div className="directory-tools"><div className="filter-tabs">{["Needs action","All jobs","Invoiced","Paid"].map(item => <button key={item} className={accountingFilter === item ? "active" : ""} onClick={() => setAccountingFilter(item)}>{item}</button>)}</div><button className="secondary" onClick={() => flash("Accounting list downloaded")}>↓ Download</button></div><div className="directory-head accounting-directory"><span>Work order</span><span>Customer / vendor</span><span>Vendor invoice</span><span>Customer invoice</span><span>Status</span></div>{rows.filter(r => accountingFilter === "All jobs" || accountingFilter === "Needs action" && ["Prepare invoice","Ready for QBO"].includes(r.status) || accountingFilter === "Invoiced" && r.status === "Customer invoiced" || accountingFilter === "Paid" && r.status === "Paid").map(r => <button className="directory-row accounting-directory" key={r.wo} onClick={() => setSelectedRow(r)}><div><strong>{r.wo}</strong><small>{r.job}</small></div><div><strong>{r.customer}</strong><small>{r.vendor}</small></div><div><strong>{r.vendorInvoice}</strong><small>Vendor → FDI</small></div><div><strong>{r.customerInvoice}</strong><small>FDI → customer</small></div><div><Badge tone={r.tone}>{r.status}</Badge><small>Open record →</small></div></button>)}</section>
    {selectedRow&&<RecordDrawer title={`${selectedRow.wo} · ${selectedRow.job}`} subtitle="Accounting handoff record" onClose={()=>setSelectedRow(null)}><div className="detail-grid"><div><small>CUSTOMER</small><strong>{selectedRow.customer}</strong></div><div><small>ASSIGNED VENDOR</small><strong>{selectedRow.vendor}</strong></div><div><small>VENDOR INVOICE</small><strong>{selectedRow.vendorInvoice}</strong></div><div><small>CUSTOMER INVOICE</small><strong>{selectedRow.customerInvoice}</strong></div></div><div className="approval-strip"><span>✓</span><div><strong>{selectedRow.status}</strong><p>Proposal, NTE authorization, vendor invoice, completion evidence, and customer billing stay connected to this work order.</p></div></div><div className="portal-actions"><button className="secondary" onClick={()=>flash("Evidence package opened")}>View evidence</button><button className="primary" onClick={()=>flash("QuickBooks handoff prepared")}>Prepare QBO handoff</button></div></RecordDrawer>}
  </div>;
}
