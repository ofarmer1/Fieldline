...
function ExternalPortal({ role, setRole, flash, notice }: { role: Exclude<Role,"fdi">; setRole: (role: Role) => void; flash: (s: string) => void; notice: string }) {
  ...
  return <main className={`external-shell ${isVendor ? "vendor-portal" : "customer-portal"}`}>
    ...
    <section className="external-content">
      ...
      <div className="portal-tabs">{(isVendor ? [["jobs","Available jobs"],["opportunity","Current job"],["messages","Messages"],["schedule","Schedule"],["evidence","Completion"]] : [["new","New request"],["requests","My requests"],["request","Current request"],["proposal","Proposal"],["schedule","Schedule"],["updates","Updates"]]).map(([id,label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}>{label}</button>)}</div>
...
function FacilityMap({ vendor=false }: { vendor?: boolean }) {
  return <aside className="portal-panel map-panel"><div className="portal-panel-title"><div><h2>{vendor?"Nearby opportunities":"Facility coverage"}</h2><p>{vendor?"Approximate locations before assignment.":"Live pilot facility locations."}</p></div></div><div className="map-canvas"><span className="road r1"/><span className="road r2"/><span className="road r3"/><button className="map-pin p-troy" title="Troy"><b>1</b></button><button className="map-pin p-royal" title="Royal Oak"><b>2</b></button><button className="map-pin p-novi" title="Novi"><b>3</b></button><div className="map-label l-troy">Troy</div><div className="map-label l-royal">Royal Oak</div><div className="map-label l-novi">Novi</div></div><div className="map-legend"><span><i/>Matched job</span><small>{vendor?"Exact address and access details unlock after assignment.":"3 facilities with active work"}</small></div></aside>;
}
